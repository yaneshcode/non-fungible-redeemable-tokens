...
contract("SoftwareLicence", function (accounts) {
  ...
  describe('Non Fungible Token Redemption', function() {
    ...
    it('Should allow whitelisted user to redeem a token', async function() {
      ...
      await this.token.redeem( { from: user1 });

      assert.equal(
        (await this.token.ownerToTokenId(user1)),
        1,
        "User has not redeemed correct token."
      );
    });

    it('Should fail if non whitelisted user tries to redeem a token', async function() {
      await shouldFail.reverting(this.token.redeem());
    });
    ...
    it('Should fail if whitelisted user tries to redeem more than one token', async function() {
      ...
      await shouldFail.reverting(this.token.redeem({ from: user1 }));
    ...
    it('Should mint a token for user if successfully redeemed token', async function() {
      ...
      assert.equal(
        (await this.token.ownerToTokenId(user1)),
        1,
        "User has not redeemed correct minted token."
      );
    ...
    it('Should not mint a token for user if unsuccesfully redeemed token', async function() {
      ...
      //await shouldFail.throwing(this.token.tokens(2));
    ...
    it('TokenId should increment by one with each redemption', async function() {
      ...
      assert.equal(
        (await this.token.whitelist(user1)),
        true,
        "User1 has not been added to whitelist after successful addition."
      );

      await this.token.addWhitelist(user2);

      assert.equal(
        (await this.token.whitelist(user2)),
        true,
        "User2 has not been added to whitelist after successful addition."
      );
      ...
      await this.token.redeem( { from: user2 });

      assert.equal(
        (await this.token.ownerToTokenId(user2)),
        2,
        "User has not redeemed correct token."
      );
    });
    ...
    it('Should fire an event on successful token redemption', async function() {
      ...
      let { logs } = await this.token.redeem( { from: user1 });
      ...
      await expectEvent.inLogs(logs, 'RedeemLicence', { recipient: user1, tokenId: '1' });
